import { CALENDAR_ASSERTION_JSON_SCHEMA } from '../src/lib/publication-calendar-pipeline.js';
    ...
    catalysts: {
      ...
      items: {
        ...
        required: [
          ...
          'whyItMatters', 'sourceIds', 'calendar',
        ],
        properties: {
          calendar: CALENDAR_ASSERTION_JSON_SCHEMA,
          date: { type: 'string' },
          event: { type: 'string' },
...
- Keep calendar entries strictly upcoming at the verified release time. Released outcomes belong in sourced highlights, not in preview-calendar rows; never backdate or relabel an outcome to force eligibility.
- For each catalyst include calendar: null when no supported adapter exists. For national BLS CPI, Producer Price Index and Employment Situation initial monthly releases, include exactly publisher BLS, series CPI/PPI/EMPSIT respectively, referencePeriod YYYY-MM, releaseStage initial, eventDate YYYY-MM-DD, releaseTime HH:mm, timeZone America/New_York, and releaseAt as a UTC ISO instant, all explicitly checked against official sources. Use the exact event label BLS Consumer Price Index (CPI) for Month YYYY, BLS Producer Price Index (PPI) for Month YYYY, or BLS Employment Situation for Month YYYY, respectively. Preserve the original official date; never guess a missing reference period or clock. Unsupported or missing calendar records will hold automatic publication.
