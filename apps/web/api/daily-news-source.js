...
- For each catalyst include calendar: null when no supported adapter exists. For national BLS CPI only, include exactly publisher BLS, series CPI, referencePeriod YYYY-MM, releaseStage initial, eventDate YYYY-MM-DD, releaseTime HH:mm, timeZone America/New_York, and releaseAt as a UTC ISO instant, all explicitly checked against official sources. Use the event label BLS Consumer Price Index (CPI) for Month YYYY. Preserve the original official date; never guess a missing reference period or clock. Unsupported or missing calendar records will hold automatic publication.
