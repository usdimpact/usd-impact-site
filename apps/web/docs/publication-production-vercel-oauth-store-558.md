...
Dormant repository-migration candidate for draft PR #559. The reviewed store schema is now represented by a real repository Supabase migration, but that migration has **not** been applied to Production. Nothing in this increment creates live database roles, schemas, rows, passwords, Vercel App credentials, OAuth tokens, Production environment variables, routes, deployments, publication authority, admissions, receipts, promotions, or merges.
...
Applying this migration to Production remains a separate protected action.
...
- explicit revocation of every publication reader/writer function governed by #558.

During migration preparation, the runtime verifier was corrected to inspect these **column-level** grants with PostgreSQL `has_column_privilege()` / `has_any_column_privilege()` rather than incorrectly requiring table-level SELECT/UPDATE. This preserves the narrower database grant instead of broadening it to satisfy the verifier.
...
- applying the prepared migration to Production;
