...
assert.match(checkoutSource, /fails closed while it\s+verifies the current Live release state/i);
assert.match(checkoutSource, /No purchase control is shown unless every\s+release gate is confirmed/i);
...
assert.match(checkoutSource, /This is the approved public-facing disclosure bundle/i);
assert.match(checkoutSource, /readiness endpoint must continue to confirm\s+every release gate/i);
...
assert.match(productSource, /verifies the current Live release state/i);
assert.match(productSource, /fails closed to the waitlist/i);
