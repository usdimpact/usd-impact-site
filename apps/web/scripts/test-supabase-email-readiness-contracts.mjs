...
assert.match(
  migration,
  /grant select, insert on table public\.marketing_consent_events to service_role;/i,
);
assert.doesNotMatch(
  migration,
...
assert.match(
  migration,
  /grant select, insert on table public\.notification_outbox to service_role;/i,
);
assert.match(
  migration,
