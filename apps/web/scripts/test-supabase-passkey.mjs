...
assert.match(browserHelper, /isConditionalMediationAvailable/);
assert.match(browserHelper, /mediation: 'conditional'/);
assert.match(browserHelper, /new AbortController\(\)/);
assert.match(browserHelper, /openEmailFallback/);
assert.match(browserHelper, /setAttribute\('autocomplete', 'email webauthn'\)/);
assert.match(browserHelper, /action=passkey&op=status/);
assert.match(browserHelper, /action=passkey&op=authentication-options/);
assert.match(browserHelper, /action=passkey&op=authentication-verify/);
assert.match(browserHelper, /X-Turnstile-Token/);
assert.match(browserHelper, /resetTurnstile\(\)/);
assert.match(browserHelper, /rememberDevice:/);
assert.match(browserHelper, /safeNextPath/);
assert.match(browserHelper, /form\?\.addEventListener\('submit', stopConditionalRequest/);
assert.match(browserHelper, /Continue with email below/);
assert.doesNotMatch(browserHelper, /localStorage|sessionStorage/);
assert.match(signInPage, /Sign in without a password/);
...
assert.match(signInPage, /Use email instead/);
assert.match(signInPage, /Keep me signed in on this device for 30 days/);
assert.match(signInPage, /Use your passkey for the quickest sign-in\. Email remains available as a backup\./);
