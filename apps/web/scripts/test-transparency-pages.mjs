...
assert.match(about, /one same-run input matrix feeds the score and reproduction bundle/);
...
assert.match(methodology, /one same-run provider snapshot feeds both the score and its reproduction bundle/);
assert.match(methodology, /complete-matrix and per-driver SHA-256 fingerprints/);
