...
function normalizeRoute(value) {
  if (typeof value !== 'string' || !value.startsWith('/')) {
    fail(`Invalid route: ${String(value)}`);
  }
...
if (!Number.isInteger(accessMap.passScore) || accessMap.passScore < 1 || accessMap.passScore > 10) {
  fail('passScore must be an integer from 1 to 10.');
}
if (!Array.isArray(accessMap.quizzes) || accessMap.quizzes.length !== 12) {
  fail('Access map must contain exactly 12 quizzes.');
}
...
const quizzes = await Promise.all(
  quizFiles.map(async (name) => ({
    name,
    data: JSON.parse(await fs.readFile(path.join(quizDir, name), 'utf8')),
  })),
);
...
for (const { name, data: quiz } of quizzes) {
  for (const key of [
    'quizId', 'canonicalId', 'language', 'title', 'slug', 'relatedLessonUrl',
    'difficulty', 'estimatedTime', 'audience', 'format', 'questionCount',
    'learningObjective', 'conceptsTested', 'frameworkNote', 'questions',
    'answerKey', 'scoreInterpretation', 'followUpReadingPath', 'complianceNote',
    'status', 'version',
  ]) {
  ...
  if (quiz.questionCount !== 10 || quiz.questions.length !== 10) {
    fail(`${name}: expected exactly 10 questions.`);
  }
  ...
  seenCanonicalIds.add(quiz.canonicalId);
  seenSlugs.add(normalizedSlug);
  quizByCanonicalId.set(quiz.canonicalId, quiz);

  const numbers = new Set();
  const keyByNumber = new Map(quiz.answerKey.map((item) => [item.question, item.correctAnswer]));
  for (const question of quiz.questions) {
    if (!Number.isInteger(question.number) || question.number < 1 || question.number > 10) {
      fail(`${name}: invalid question number ${question.number}`);
    }
    ...
    if (!['multiple-choice', 'true-false'].includes(question.type)) {
      fail(`${name}: unsupported question type ${question.type}`);
    }
    if (!Array.isArray(question.options) || question.options.length < 2) {
      fail(`${name}: question ${question.number} needs at least two options.`);
    }
    const optionKeys = new Set(question.options.map((option) => option.key));
    if (optionKeys.size !== question.options.length) {
      fail(`${name}: question ${question.number} has duplicate option keys.`);
    }
    if (!optionKeys.has(question.correctAnswer)) {
      fail(`${name}: question ${question.number} correctAnswer is not an option.`);
    }
    if (keyByNumber.get(question.number) !== question.correctAnswer) {
      fail(`${name}: question ${question.number} does not match answerKey.`);
    }
    if (!question.explanation || !question.sourceReference) {
      fail(`${name}: question ${question.number} needs an explanation and sourceReference.`);
    }
  }
}
...
for (const access of accessMap.quizzes) {
  ...
  if (normalizeRoute(access.slug) !== normalizeRoute(quiz.slug)) {
    fail(`${access.canonicalId}: access-map slug does not match quiz.`);
  }
  if (normalizeRoute(access.relatedLessonUrl) !== normalizeRoute(quiz.relatedLessonUrl)) {
