import { renderAudiobookPlayerScript } from './audiobook-player-client.js';
...
export function renderProtectedAudiobook() {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Read the Dollar First English Audiobook | USD Impact</title><style>:root{--navy:#071a33;--gold:#c9a35b;--paper:#f5f7fa;--ink:#101923;--line:#d7dde5}*{box-sizing:border-box}body{margin:0;background:var(--paper);color:var(--ink);font-family:Inter,system-ui,sans-serif}header,footer{background:var(--navy);color:#fff}.wrap{max-width:1080px;margin:auto}.site-nav-wrap{display:flex;align-items:center;justify-content:space-between;gap:24px;padding:18px 22px}.site-brand{color:#fff;font-weight:850;text-decoration:none}.site-nav{display:flex;align-items:center;gap:8px;flex-wrap:wrap}.site-nav a{min-height:42px;display:inline-flex;align-items:center;padding:8px 10px;border-radius:9px;color:#dce4ee;font-size:.9rem;font-weight:700;text-decoration:none}.site-nav a:hover,.site-nav a:focus-visible,.site-nav a[aria-current=page]{background:rgba(255,255,255,.1);color:#fff}.site-nav a:focus-visible{outline:3px solid var(--gold);outline-offset:2px}.hero{background:#0b2443;color:#fff;padding:54px 22px}main{padding:34px 22px 64px}.card{background:#fff;border:1px solid var(--line);border-radius:18px;padding:28px;margin-bottom:22px}audio{width:100%;margin:16px 0}button,select{font:inherit}button{cursor:pointer}.controls{display:flex;gap:10px;align-items:center;flex-wrap:wrap}.controls label{margin-left:auto}ol{display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:0;list-style:none}li button{width:100%;min-height:58px;text-align:left;border:1px solid var(--line);border-radius:10px;background:#f8fafc;padding:10px}li button[aria-current=true]{border-color:var(--gold);background:#fff8e8}time{float:right;color:#5a6472}.note{color:#5a6472}.compliance{font-size:.9rem;color:#5a6472}@media(max-width:720px){.site-nav-wrap{align-items:flex-start;flex-direction:column;gap:10px}.site-nav{width:100%;gap:4px}.site-nav a{padding:7px 8px}ol{grid-template-columns:1fr}.controls label{margin-left:0}}</style>${memberMainMenuAssets()}
</head><body><header><div class="wrap site-nav-wrap"><a class="site-brand" href="/" aria-label="USD Impact home">USD Impact</a><nav class="site-nav" aria-label="Library navigation"><a href="/guided-edition/">Guided Edition</a><a href="/guided-edition/book/">Book</a><a href="/guided-edition/audiobook/" aria-current="page">Audiobook</a><a href="/guided-edition/video-library/">Video Library</a><a href="/account/">Account</a></nav>${renderMemberMainMenu()}</div></header><section class="hero"><div class="wrap"><p>Protected Library Pass audiobook</p><h1>Read the Dollar First</h1><p>The complete 20-track English edition. Your listening position is stored only in this browser.</p></div></section><main class="wrap" data-player data-key="usd-impact-library-pass-audiobook-progress"><section class="card"><h2>Now listening</h2><p class="note">AI-generated speech with human quality review.</p><strong data-title>${escapeHtml(first.title)}</strong><audio data-audio src="${escapeHtml(privateAudiobookTrackHref(first))}" preload="metadata" controls>Audio playback is not supported.</audio><div class="controls"><button type="button" data-prev disabled>Previous chapter</button><button type="button" data-next>Next chapter</button><button type="button" data-retry>Retry chapter</button><button type="button" data-restart>Start chapter over</button><label>Speed <select data-speed><option value="0.75">0.75×</option><option value="1" selected>1×</option><option value="1.25">1.25×</option><option value="1.5">1.5×</option><option value="2">2×</option></select></label></div><p class="note" data-status role="status" aria-live="polite">Listening progress will be saved on this device when available.</p></section><section class="card"><h2>Chapters</h2><ol>${tracks}</ol></section><p class="compliance"><strong>Educational and informational only.</strong> Not investment, financial, legal, tax or trading advice; not a trading signal or recommendation. Market relationships are regime-dependent and may change.</p></main><footer><div class="wrap">USD Impact · Protected learning library</div></footer><script>${renderAudiobookPlayerScript()}</script></body></html>`;
}
