...
// Provider-managed auth is forbidden in the application outbox.
// Keep the exact observed sender and approved subjects paired; sender/subject
...
const AUTH_SUBJECTS = new Set([
  'Your secure USD Impact sign-in link',
...
export function isProviderManagedAuthSuccess(event) {
  ...
  if (data.from !== AUTH_FROM || !AUTH_SUBJECTS.has(data.subject)) return false;
