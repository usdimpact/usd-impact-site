...
const SCHEMA_HOLD = 'HOLD_TREASURY_BUYBACK_OUTCOME_DISCOVERY_SCHEMA';
...
function skipRawElement(document, tagName, openingStart, openingEnd, limit) {
  const closing = findTag(document, tagName, openingEnd, limit, true);
  if (!closing) hold(SCHEMA_HOLD, `TreasuryDirect buyback table contains an unclosed ${tagName} element.`);
  return closing.end;
}
...
function stripMarkup(document, start, limit) {
  ...
  while (cursor < limit) {
    ...
    if (character === '<') {
      const script = findTag(document, 'script', cursor, Math.min(limit, cursor + 16), false);
      if (script?.start === cursor) {
        cursor = skipRawElement(document, 'script', script.start, script.end, limit);
        pendingSpace = output.length > 0;
        continue;
      }
      const style = findTag(document, 'style', cursor, Math.min(limit, cursor + 16), false);
      if (style?.start === cursor) {
        cursor = skipRawElement(document, 'style', style.start, style.end, limit);
        pendingSpace = output.length > 0;
        continue;
      }
...
function canonicalArtifactHref(value) {
  ...
  if (year.length !== 4 || !year.startsWith('20') || !asciiDigits(year) || !filename.endsWith('.xml')) return null;
  const stem = filename.slice(0, -4);
  const separator = stem.indexOf('_');
  if (separator <= 0 || separator !== stem.lastIndexOf('_')) return null;
  const prefix = stem.slice(0, separator);
  const operationStamp = stem.slice(separator + 1);
  if (!new Set(['BBPA', 'BBA', 'BBR']).has(prefix) || operationStamp.length !== 14
      || !operationStamp.startsWith('20') || !asciiDigits(operationStamp) || year !== operationStamp.slice(0, 4)) return null;
  return Object.freeze({ url: url.toString(), prefix, operationStamp });
}

function artifactFromCell(document, cell, expectedPrefix) {
  const links = [];
  let cursor = cell.innerStart;
  while (cursor < cell.innerEnd) {
    const anchor = findTag(document, 'a', cursor, cell.innerEnd, false);
    if (!anchor) break;
    const attributes = parseOpeningTagAttributes(document.source, anchor.start, anchor.end, 'a');
    const parsed = canonicalArtifactHref(attributes.get('href'));
    if (parsed) links.push(parsed);
    if (links.length > 2) {
      hold('HOLD_TREASURY_BUYBACK_OUTCOME_DISCOVERY_AMBIGUOUS', `TreasuryDirect ${expectedPrefix} cell contains too many canonical XML artifacts.`);
    }
    cursor = anchor.end;
  }
  if (links.length === 0) {
...
function parseListing(body, operationDate) {
  ...
  for (const row of rows) {
    ...
    if (cells.length < 10 || stripMarkup(document, cells[0].innerStart, cells[0].innerEnd) !== expectedDate) continue;

    const artifacts = {};
    for (const stage of STAGE_CELLS) {
      if (stage.index >= cells.length) {
        hold(SCHEMA_HOLD, 'TreasuryDirect buyback row no longer exposes the reviewed announcement/result columns.');
      }
      artifacts[stage.name] = artifactFromCell(document, cells[stage.index], stage.prefix);
    }
