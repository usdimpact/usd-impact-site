...
Published content is stored as versioned JSON in `public.guided_content_releases`. The table has forced row-level security, an explicit deny policy for browser roles, no client privileges, and a read-only grant for the server-side `service_role`. The protected server route loads the release only after account entitlement succeeds, then checks both the source-document hash and reader-text hash before rendering. Interactive questions remain a separate learning layer inside the protected payload and are not presented as canonical manuscript text.
