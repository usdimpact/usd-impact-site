...
import { dailyFreshness, probe, repositoryContracts, vercelContracts } from './integrity-watchdog-collectors.mjs';
...
const safe = sanitize({ email: 'person@example.com', authorization: 'Bearer abcdefghijklmnopqrstuvwxyz', nested: openAiShape, secret_values_persisted: false });
...
const headers = { 'strict-transport-security': 'max-age=63072000', 'x-content-type-options': 'nosniff', 'x-frame-options': 'DENY', 'cache-control': 'no-store' };
const goodProbe = await probe({ fetchImpl: async () => reply('required marker', 200, headers, 'https://example.test/'), id: 'HTTP-PASS', workflowId: 'WF', title: 'HTTP pass', severity: SEVERITY.P1, url: 'https://example.test/', requiredText: ['required marker'], requiredHeaders: headers, goldEligible: true });
assert.equal(goodProbe.outcome, OUTCOME.PASS);
assert.equal(goodProbe.classification, CLASSIFICATION.GOLD_CANDIDATE);
const badProbe = await probe({ fetchImpl: async () => reply('wrong', 503), id: 'HTTP-FAIL', workflowId: 'WF', title: 'HTTP fail', severity: SEVERITY.P0, url: 'https://example.test/', expectedStatus: 200, requiredText: ['required marker'] });
assert.equal(badProbe.outcome, OUTCOME.FAIL);

const currentDaily = await dailyFreshness({ fetchImpl: async () => reply('<a href="/news/2026-09-04/">Daily</a>'), baseUrl: 'https://www.usd-impact.com', now: new Date('2026-09-04T18:00:00Z') });
const staleDaily = await dailyFreshness({ fetchImpl: async () => reply('<a href="/news/2026-09-03/">Daily</a>'), baseUrl: 'https://www.usd-impact.com', now: new Date('2026-09-04T18:00:00Z') });
...
const reviewed = await independentReview({
  results: [passed], projectSummary: { health: 'GREEN' }, enabled: true,
  env: { USDIMPACT_WATCHDOG_OPENAI_API_KEY: openAiShape, USDIMPACT_WATCHDOG_OPENAI_MODEL: 'gpt-5.6-terra' },
  fetchImpl: async (url, options) => {
    request = { url: String(url), options };
    return reply(JSON.stringify({ id: 'resp_test', output: [{ content: [{ type: 'output_text', text: JSON.stringify({ verdict: 'CONFIRM', summary: 'Supported.', challenged_contract_ids: [], findings: [] }) }] }] }), 200);
  },
});
