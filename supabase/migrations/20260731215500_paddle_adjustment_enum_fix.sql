...
begin
  ...
  if p_adjustment_type is null or char_length(p_adjustment_type) > 32 then
  ...
  if p_adjustment_total_cents < v_purchase_total_cents then
  ...
  v_event_key := 'paddle:adjustment:' || p_adjustment_id || ':approved-full-amount';
  ...
  set status = case
        when p_action = 'chargeback' then 'charged_back'::public.purchase_status
        else 'refunded'::public.purchase_status
      end,
      refunded_at = case
        when p_action = 'refund' then coalesce(p_occurred_at, now())
        else refunded_at
      end,
      disputed_at = case
        when p_action = 'chargeback' then coalesce(p_occurred_at, now())
        else disputed_at
      end,
  ...
  insert into public.entitlement_events (
    ...
    v_entitlement.state,
    case
      when p_action = 'chargeback' then 'approved full-amount Paddle chargeback'
      else 'approved full-amount Paddle refund'
    end,
    'paddle_webhook',
    p_adjustment_id,
    p_event_id,
...
revoke all on function public.apply_paddle_access_revocation(
  text, timestamptz, text, text, text, integer, text, jsonb
) from public, anon, authenticated;
grant execute on function public.apply_paddle_access_revocation(
  text, timestamptz, text, text, text, integer, text, jsonb
) to service_role;
