...
-- Learning progress is readable by its owner, but writes are mediated by the
-- verified server route. This prevents a browser from assigning itself a
-- mastery score, completion status, or attempt count through the Data API.
...
alter table public.learning_progress
  add constraint learning_progress_content_id_length
    check (length(content_id) between 1 and 160),
  add constraint learning_progress_resume_position_length
    check (resume_position is null or length(resume_position) between 1 and 80),
  add constraint learning_progress_data_size
...
begin
  ...
  on conflict (account_id, content_id) do update
    set status = case
        ...
        progress_percent = greatest(target.progress_percent, excluded.progress_percent),
...
grant select, insert, update on public.learning_progress to service_role;
