...
begin
  ...
  -- Profile status is read-only here. Do not use FOR UPDATE: under SECURITY INVOKER,
  -- PostgreSQL requires UPDATE privilege for that row lock, and service_role must not
  -- receive broad profile-write permission merely to reserve a commerce intent.
